...
pub fn generate(
    ...
    return switch (out_lang) {
        ...
        .msl => try genMsl(allocator, air, debug_info, entrypoint.?, bindings.?),
        .glsl => try genGlsl(allocator, air, debug_info, entrypoint.?, bindings.?),

        // SPIR-V
        else => {
            // TODO - air.tree.source points to free'd memory
            ...
            const spirv_words = @as([*]const u32, @ptrCast(@alignCast(spirv_data_z.ptr)))[0 .. spirv_data_z.len / @sizeOf(u32)];
            ...
            const res = c.spvOptimizerRun(optimizer, spirv_words.ptr, spirv_words.len, &optimized_spirv, opt_options);
            switch (res) {
                c.SPV_SUCCESS => {},
            ...
            if (out_lang == .spirv)
                return allocator.dupe(u8, @as([*]const u8, @ptrCast(optimized_spirv.*.code))[0 .. optimized_spirv.*.wordCount * @sizeOf(u32)]);
            ...
            switch (out_lang) {
                .glsl => {
                    ...
                    for (resource_types) |resource_type| {
                        glslRemapResources(compiler, resources, resource_type, bindings.?);
                    }
                    ...
                    _ = c.spvc_compiler_set_entry_point(compiler, entrypoint.?.name, spvExecutionModel(entrypoint.?.stage));
                },
                else => {},
            }
...
fn spvcErrorCallback(userdata: ?*anyopaque, err: [*c]const u8) callconv(.C) void {
    _ = userdata;
    // TODO - don't panic
    std.debug.panic("{s}", .{std.mem.span(err)});
}
