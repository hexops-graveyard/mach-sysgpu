...
pub const Impl = dgpu.Interface(struct {
    ...
    pub inline fn bufferGetSize(buffer_raw: *dgpu.Buffer) u64 {
        const buffer: *impl.Buffer = @ptrCast(@alignCast(buffer_raw));
        return buffer.getSize();
    }

    pub inline fn bufferGetUsage(buffer_raw: *dgpu.Buffer) dgpu.Buffer.UsageFlags {
        const buffer: *impl.Buffer = @ptrCast(@alignCast(buffer_raw));
    ...
    pub inline fn bufferSetLabel(buffer_raw: *dgpu.Buffer, label: [*:0]const u8) void {
        const buffer: *impl.Buffer = @ptrCast(@alignCast(buffer_raw));
    ...
    pub inline fn deviceCreateSampler(device_raw: *dgpu.Device, descriptor: ?*const dgpu.Sampler.Descriptor) *dgpu.Sampler {
        const device: *impl.Device = @ptrCast(@alignCast(device_raw));
        const sampler = device.createSampler(descriptor orelse &dgpu.Sampler.Descriptor{}) catch @panic("api error");
